...
uint16_t Sensor1_data  = 0x1111;		 /* 假设定义的传感器数据 */	
uint16_t Sensor2_data  = 0x2222; 
uint16_t Sensor3_data  = 0x3333; 
uint16_t Sensor4_data  = 0x4444; 
uint16_t Sensor5_data  = 0x5555; 
uint16_t Sensor6_data  = 0x6666; 
uint16_t Sensor7_data  = 0x7777;		 /* 假设定义的传感器数据 */	
uint16_t Sensor8_data  = 0x8888; 
uint16_t Sensor9_data  = 0x9999; 
uint16_t Sensor10_data = 0xaaaa; 
uint16_t Sensor11_data = 0x1111; 
uint16_t Sensor12_data = 0x2222; 
uint16_t Sensor13_data = 0x3333;		 /* 假设定义的传感器数据 */	
uint16_t Sensor14_data = 0x4444; 
uint16_t Sensor15_data = 0x5555; 
uint16_t Sensor16_data = 0x6666; 
uint16_t Sensor17_data = 0x7777; 
uint16_t Sensor18_data = 0x8888; 
uint16_t Sensor19_data = 0x9999; 
uint16_t Sensor20_data = 0xaaaa; 
...
void MY_UART1_Idle_Callback(uint8_t size)
{
	...
	if(test_crc_sum(uart1_rec_buf,size) != uart1_rec_buf[size-1])		/* 判断crc */
	{
		#if PRINTF_UART1
			printf("crc error\r\n");
		#endif
	...
	if(rec_header == UART1_FRAME_HANDER)					/* 判断帧头 */
	{
...
//				{
					...
					uart1_send_ask_buf[0] = UART1_FRAME_HANDER >> 8;	/* 定义发送帧头 */
					uart1_send_ask_buf[1] = (uint8_t)UART1_FRAME_HANDER;
					uart1_send_ask_buf[2] = UART1_ASK_CMD;						/* 定义数据命令字 */
				
					/* 从第3位开始就是自己添加的内容了*/
					uart1_send_ask_buf[3] = 0x05;								
					uart1_send_ask_buf[4] = 0x01;				
					uart1_send_ask_buf[5] = 0x02;								
					uart1_send_ask_buf[6] = 0x03;								
					uart1_send_ask_buf[7] = 0x04;
					uart1_send_ask_buf[8] = 0x05;
					uart1_send_ask_buf[9] = get_crc_sum(uart1_send_ask_buf, 9);
					uart1_send_ask_buf[10] = '\0';
					printf("%s",uart1_send_ask_buf);
//				}
			}
		}
		...
		else if(uart1_rec_buf[2] == UART1_DATA_CMD)	/* 判断ROS发送来的是否为获取传感器数据命令 */
		{
			...
			uint8_t count =4;													/* 传感器数据在数组中的起始位,应该为4*/
			uart1_send_data_buf[0] = UART1_FRAME_HANDER >> 8;				/* 定义发送帧头 */
			uart1_send_data_buf[1] = (uint8_t)UART1_FRAME_HANDER;
			uart1_send_data_buf[2] = UART1_DATA_CMD;								/* 定义数据命令字 */
			uart1_send_data_buf[3] = rec_code_num*2;								/* 定义发送数据长度 */
			
			#if PRINTF_UART1
				uint8_t rec_code_buf[6];											
				for(int i=0; i<rec_code_num; i++)			
				{
					rec_code_buf[i] = uart1_rec_buf[3+1+i];	
				}
				printf("接收到%d个命令字\r\n", rec_code_num);
				for(int i=0; i<rec_code_num; i++)
				{
			...
			uart1_send_data_buf[count] = get_crc_sum(uart1_send_data_buf, count);
			uart1_send_data_buf[count+1] = '\0';
