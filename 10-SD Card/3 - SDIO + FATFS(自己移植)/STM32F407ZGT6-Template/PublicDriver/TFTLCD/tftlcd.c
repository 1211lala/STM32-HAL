...
#include ".\TFTLCD\lcdfont.h"

/* lcd_ex.c存放各个LCD驱动IC的寄存器初始化部分代码,以简化lcd.c,该.c文件
 * 不直接加入到工程里面,只有lcd.c会用到,所以通过include的形式添加.(不要在其他文件再包含该.c文件!!否则会报错!)
 */
#include ".\TFTLCD\lcd_ex.c"

SRAM_HandleTypeDef g_sram_handle; /* SRAM句柄(用于控制LCD) */

/* LCD的画笔颜色和背景色 */
uint32_t g_point_color = RED;  /* 画笔颜色 */
uint32_t g_back_color = WHITE; /* 画笔背景色 */
 ...
 * @param       mode=0 竖屏 mode=1 横批
 * @param       color  设置刷屏颜色
 * @retval      无
 */
void lcd_init(uint8_t mode, uint16_t color)
{
    // GPIO_InitTypeDef gpio_init_struct;
 ...
 * @retval      无
 */
void lcd_show_string(uint16_t x, uint16_t y, uint8_t size, char *p, uint16_t color)
{
    /**********************************/
    uint16_t height = size;
    uint16_t width = size / 2 * strlen(p);
    /*********************************/
    uint8_t x0 = x;
    width += x;
    height += y;
