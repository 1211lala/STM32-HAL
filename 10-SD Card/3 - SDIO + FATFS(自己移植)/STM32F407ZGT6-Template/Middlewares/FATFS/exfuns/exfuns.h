...
/* exfuns_file_type返回的类型定义 根据表FILE_TYPE_TBL获得.在exfuns.c里面定义 */
...
extern DIR* FSdir;
extern FILINFO	*FSinfo;
extern FIL *FSFile;
extern FRESULT fs_status;
extern uint8_t *fs_buf;
extern uint32_t fsize;  
extern uint32_t fs_r_count; 
extern uint32_t fs_w_count;
...
uint8_t *exfuns_get_src_dname(uint8_t *dpfn);                               /* 得到路径下的文件夹 */
