 ...
 * @file        exfuns.c
 * @version     V1.0
 * @brief       FATFS 扩展代码 自己修改了一点正点原子的代码，不完全一样
 ****************************************************************************************************
 */
...
FATFS *SDFATFS = NULL;
FATFS *FLASHFATFS = NULL;
FIL *FSFile = NULL;
FILINFO	*FSinfo;
FRESULT fs_status = FR_OK;
DIR* FSdir;
uint8_t *fs_buf = NULL;  /* FS的读写缓存buf */


uint32_t fsize = 0;      /* 保存文件实际大小 */
uint32_t fs_r_count = 0; /* 保存实际读了多少数据 */
uint32_t fs_w_count = 0; /* 保存实际写入了多少数据 */
 ...
 */
uint8_t fs_exfuns_mem_init(void)
{
	SDFATFS = mymalloc(SRAMEX, sizeof(FATFS));
	FLASHFATFS = mymalloc(SRAMEX, sizeof(FATFS));
	FSFile = mymalloc(SRAMEX, sizeof(FIL));
	FSinfo = mymalloc(SRAMEX, sizeof(FILINFO));
	FSdir = mymalloc(SRAMEX, sizeof(DIR));
	fs_buf = mymalloc(SRAMEX, 1 * 1024);

	if (SDFATFS == NULL | FSFile == NULL | FLASHFATFS == NULL | fs_buf == NULL| FSinfo == NULL | FSdir == NULL)
	{
			return FAIL;
	}
	return OK;
