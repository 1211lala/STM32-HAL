...
uint16_t color_buf[] = {0xFFFF, 0x0000, 0x001F, 0XF81F, 0XFFE0, 0X07FF, 0xF800, 0xF81F, 0x07E0,0x7FFF, \
												0xFFE0, 0XBC40, 0XFC07, 0X8430, 0X01CF, 0X7D7C, 0X5458, 0X841F, 0XEF5B, 0XC618, 0XA651, 0X2B12};
/* LCD缓存大小设置，修改此值时请注意！！！！
 *修改这两个值时可能会影响以下函数	LCD_Clear/LCD_Fill/LCD_DrawLine
 */
 
#define LCD_TOTAL_BUF_SIZE	(240*135*2)

#define LCD_Buf_Size  1024 * 10
...
//    {
//			LCD_SPI_Send(lcd_buf, LCD_Buf_Size);		/* 这个地方会超范围，但是因为是清满屏所以没问题 */
//    }
...
void LCD_Fill(uint16_t x_start, uint16_t y_start, uint16_t x_end, uint16_t y_end, uint16_t color)
{
    ...
    uint32_t size = (x_end - x_start + 1) * (y_end - y_start + 1) * 2;		/* 得出缓存大小 带颜色 */

    if(size > LCD_Buf_Size)	/* 如果需要的缓存大于给的缓存 */
    {
        size_remain = size - LCD_Buf_Size;	/* 剩下的没有被写入的大小 */
        size = LCD_Buf_Size;								/* 写入给定的最大值 */
    }

    LCD_Address_Set(x_start, y_start, x_end, y_end);	/* 设置行列地址 */

    while(1)
    {
        for(uint16_t i = 0; i < size / 2; i++)		/* 得出多少点要被写 16bit的颜色 */
        {
            lcd_buf[2 * i] = color >> 8;		/* [0]颜色低位   [1]颜色高位		[2]颜色低位   [3]颜色高位			*/
            lcd_buf[2 * i + 1] = color;
        }
