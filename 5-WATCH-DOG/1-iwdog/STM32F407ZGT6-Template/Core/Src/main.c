...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	printf("独立看门狗的时钟来自外部hsi为32.768KHz,设置了64分频为512Hz,及计一个数0.001953s\r\n");
	printf("独立看门狗的计数值设置的为4000 独立看门狗的触发时间为 0.001953*4000= 7.8s \r\n");
  ...
  while (1)
  {
		...
		if(key == key_up_down)
		{
			...
			lcd_show_string(10, 10+24*4, 24, "Iwdog Refresh!!!", BLACK);
			iwdog_count = 8;
		}
		if(key == key0_down)
		{
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	...
	if(htim == &htim7)
	{
		char buf[50];
		iwdog_count -= 1;
		sprintf(buf, "system will restart in %2ds", iwdog_count);
		lcd_show_string(10, 10+24*6, 32, buf, BLACK);
	}
}
